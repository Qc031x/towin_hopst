    ...
    $.fn.zyUpload = function (options, param) {
        ...
        return this.each(function () {
            ...
            var defaults = {
                ...
                onSuccess: function (responseInfo) {
                },            // 文件上传成功的回调方法
            ...
            this.createHtml = function () {
                ...
                var html = '';
                // 创建不带有拖动的html
                html += '<form id="uploadForms" action="' + para.url + '" method="post" enctype="multipart/form-data">';
                ...
                //html += '			<div id="fileSubmit" class="upload_submit_btn">确认上传</div>';
                //html += '		</div>';
                html += '		</div>';
...
//	            		jqArr.push($(this).attr('id'));
//	            		 console.log(jqArr.toString());
//	            	}
//	            });
//	            for(var numIndex=0;numIndex<jqArr.length;numIndex++){
//	            	$('#'+jqArr[numIndex]).find(".num_current").text(numIndex+1);
//	            	 console.log(jqArr[numIndex]);
//	            }
            ...
            this.funDisposePreviewHtml = function (file, e) {
                ...
                if (file.type.indexOf("image") == 0) {
                    html += '<div id="uploadList_' + file.index + '" class="upload_append_list" data-number="">';
                    html += '			<p><span class="file_info">sdsd</span><span><em class="num_current"></em>/<em class="num_all"></em></span></p>';
                    html += '	<a class="imgBox">';
                    html += '<span class="file_del" data-index="' + file.index + '" title="删除"><span class="del"><img src="images/eledelete.png"/></span></span>';
                    html += '		<div class="uploadImg" style="">';
                    html += '			<img id="uploadImage_' + file.index + '" class="upload_image" src="' + e.target.result + '" style="" />';
                    html += '		</div>';
                    html += '	</a>';
                    html += '	<div id="uploadProgress_' + file.index + '" class="file_progress mask"></div>';
                    ...
                    html += '<span class="file_del" data-index="' + file.index + '" title="删除"><span class="del"><img src="images/eledelete.png"/></span></span>';
                    html += '		<div class="uploadImg" style="">';
                    html += '			<img id="uploadImage_' + file.index + '" class="upload_image" src="' + fileImgSrc + '" style="" />';
                    html += '		</div>';
                    html += '	</a>';
                    html += '	<div id="uploadProgress_' + file.index + '" class="file_progress mask"></div>';
                    html += '	<p id="uploadFailure_' + file.index + '" class="file_failure">上传失败，请重试</p>';
            ...
            this.createCorePlug = function () {
                var params = {
                    ...
                    onSelect: function (selectFiles, allFiles) {
                        ...
                        var funAppendPreviewHtml = function (html) {
                            ...
                            if (para.dragDrop) {
                                ...
                                $('#uploadForm').scrollTop($('#uploadForm')[0].scrollHeight);
                            } else {
                    ...
                    onProgress: function (file, loaded, total) {
                        if ($(".maskLoad").css('display') == "none")
                            $('.mask').css('height', $('body').height() + 'px');
                        $("#uploadProgress_" + file.index).css('display', 'block');
