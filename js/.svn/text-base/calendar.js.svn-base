...
function Calendar(){
	...
	var initRiData =function(year,month,thisDay,startDay){
		...
		var type=$("#calendarType").val(); //1 商城日历  2平台日历
		var url = "";
		if(type == undefined || type == '2'){
			url = "/core/product.getRiDataByEsid.do";
		}else{
			url = "/core/saleConfirm.getRiDataByEsid.do";
		}
		jQuery.post(url,
			{
				...
				if(calendar.sourceType==8){
					...
					//单日策略点击
					$(".yue_rili .yue_day li a").click(function(){
						getDayPolicy($(this).parents("li").attr("date"));
					})
				}
	...
	var getRevDayText =function(text,temp,tempData,count){
		...
		if(calendar.sourceType==8){
			...
			textIsRev= "<li class='future' date="+temp+">"+ i +"<br>"+" <span id=ykbackrl_"+temp+"  class='zt1'>"+textIsRev+"</span><a  href='#tanbianjidiv'><em>编辑</em></a>"+"</li>"; //康良玉
			// " <span id=ykbackrl_"+temp+"  class='zt1'>"+textIsRev+"</span>";
			if(text=='2'){

				textIsRev="休息";
				textIsRev= "<li class='disfuture' date="+temp+">"+ i +"<br>"+"<span id=ykbackrl_"+temp+"  class='zt2'>"+textIsRev+"</span><a  href='#tanbianjidiv'><em>编辑</em></a>"+"</li>";
			...
			if(text=='1'){
				...
				textIsRev="可约";
				textIsRev= "<li class='future' date="+temp+">"+ i +"<br>"+" <span id=ykbackrl_"+temp+"  class='zt1'>"+textIsRev+"</span><a  href='#tanbianjidiv'><em>编辑</em></a>"+"</li>";
				;
			}
		}else{
